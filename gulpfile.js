...
// 無加工複製
task('duplicate', () => {
  return src(Cfg.duplicate.src, { since: lastRun('duplicate') })
...
task('ejs', () => {
  return (
    src(Cfg.ejs.src, { since: lastRun('ejs') })
      ...
      .pipe(
        gulpEjs({}, { async: false }).on('error', function errorHandler(error) {
...
task('sass', () => {
  return (
    src(Cfg.scss.src, { sourcemaps: Cfg.scss.mapFlg })
      ...
      // sassコンパイル
      // オプションで吐き出す形式を設定
      .pipe(gulpSass.sync({ outputStyle: Cfg.scss.outputStyle }).on('error', gulpSass.logError))
      .pipe(
        gulpPostcss([
